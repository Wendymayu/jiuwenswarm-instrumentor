...
async def test_skill_tool_load_enriches_span(exporter):
    ...
    await Fake()._execute_single_tool_call(_SkillToolCall("skill_tool"), session=_Session())
...
async def test_skill_complete_release_enriches_span_and_duration(exporter):
    ...
    inst = Fake()
    instrument_tool(tracer, metrics, ability_cls=Fake)
    await inst._execute_single_tool_call(_SkillToolCall("skill_tool"), session=_Session())
    await inst._execute_single_tool_call(_SkillToolCall(
