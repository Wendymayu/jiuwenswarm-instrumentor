...
from jiuwenswarm_instrumentor.instrumentors import llm, tool, agent, session, logs, gateway, agentserver, context_compaction, subagent
...
def apply_instrumentors(tracer, meter, cfg):
    ...
    for label, fn in (
        ...
        ("agent", lambda: agent.instrument_agent(tracer, metrics, log_messages=log_messages, message_max_length=getattr(cfg, "message_max_length", 4096))),
    ...
    if getattr(cfg, "traces_exporter", "none") != "none":
        if getattr(cfg, "instrument_gateway", True):
            try:
            ...
            logger.info("[instrumentor] gateway disabled (OTEL_INSTRUMENT_GATEWAY=false) — skipping")
        try:
        ...
        try:
            subagent.instrument_subagent(tracer, metrics)
            logger.info("[instrumentor] applied subagent")
        except Exception:
            logger.exception("[instrumentor] failed to apply subagent — skipping")
